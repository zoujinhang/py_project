...
from Data_analysis import ch_to_energy,TD_bs
...
indexi = np.where((bin_c>=609725989.199308)&(bin_c<=609725989.64913))[0]
t_v = bin_c[indexi]
...
locate = Locate(Fermi)
ra,dec,err_r,ra_rcat,dec_rcat,chi2 = locate.condidate(t_m,np.array(dete_rate),np.array(dete_bs),'case1',[5,50])
...
smp.savefig(savedir+'A_sky_map_100.png')
smp.close()
