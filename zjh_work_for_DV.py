...
ax1.plot(t_c1,rate1,label = r'$R_{%.2fs}$' % binsize1)
ax1.set_ylim([10000,20000])
ax1.set_xlim([0,600])
ax1.set_xticks([])
ax1.axhline(y = mean1,color = 'r',label = r'$Mean_{%.2fs} = %.2f$' % (binsize1,mean1))
ax1.axhline(y = sigma1+mean1,color = '#f47920',label = r'$\sigma_{%.2fs} = %.2f$' % (binsize1,sigma1))#大
...
ax2.plot(t_c2,rate2,label = r'$R_{%.2fs}$' % binsize2)
ax2.set_ylim([10000,20000])
ax2.set_xlim([0,600])
ax2.axhline(y = mean2,color = 'r',label = r'$Mean_{dt = %.2fs} = %.2f$' % (binsize2,mean2))
ax2.axhline(y = sigma2+mean2,color = 'g',label = r'$\sigma_{%.2fs} = %.2f$' % (binsize2,sigma2))
ax2.axhline(y = mean2-sigma2,color = 'g')
ax2.axhline(y = mean1+sigma1,color = '#f47920',label =r'$\sigma_{%.2fs}$ = %.2f' % (binsize1,sigma1))
...
fig.savefig(savedir + 'A_sample_rate1.pdf')
plt.close(fig)
...
'''
plt.figure()
plt.plot(t_c2,rate2,label = r'$R_{%.2fs}$' % binsize2)
plt.xlim([0,600])
plt.axhline(y = mean2,color = 'r',label = r'$Mean_{%.2fs} = %.2f$' % (binsize2,mean2))
plt.axhline(y = sigma2+mean2,color = 'g',label = r'$\sigma_{%.2fs} = %.2f$' % (binsize2,sigma2))
plt.axhline(y = mean2-sigma2,color = 'g')
...
plt.axhline(y = mean1+sigma1,color = '#f47920',label = r'$\sigma_{ %.2fs}$ = %.2f' % (binsize1,sigma1))
plt.axhline(y = mean2-sigma2_1,color = 'k')
plt.axhline(y = mean1-sigma1,color = '#f47920')
plt.xlabel('t')
plt.ylabel('rate')
plt.legend()
plt.savefig(savedir + 'A_sample_rate2.pdf')
plt.close()
